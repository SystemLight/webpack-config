...
export class FileInfo {
  ...
  public id: string
  public parent: FileInfo | null
  public layout: FileInfo | null
  public name: string
  public nameWithExt: string
...
export class AutoRoute {
  ...
  public ignoreFiles: RegExp[]
  public ignoreFolders: RegExp[]

  private readonly _layout: string
  private readonly _routesRender: (routes: Routes) => string
  ...
  getFileInfos() {
    let inputInfos: FileInfo[] = [new FileInfo(this.targetPath)]
    let outputInfos: FileInfo[] = []
   ...
   * @param {Boolean} exact
   */
  getFirstLevelRoute(info, exact = true): Route {
    ...
    for (let info of infos) {
      if (info.layout) {
        ...
        while (current.layout) {
          ...
          if (!layoutRoute['routes'].includes(route)) {
...
export class AutoRouteWebpackPlugin {
  static loader = require.resolve('./loader.js')

  public id: string

  private _loader: {test: RegExp; options: any}
  private _autoRoute: AutoRoute
...
export {vueRoutesRender} from './vue-routes-render'
